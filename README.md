...
> Ограничение: ленивый режим ограничивает память **значений**. Метаданные —
> иерархия и геометрия блоков — грузятся целиком, потолка у них нет, и
> `cacheBytes` на них не влияет. Цена — около 230 байт на узел, то есть дизайн
> с миллионом узлов стоит ~220 МиБ до того, как прочитано первое значение.
> Сколько это для конкретной БД, отвечает `Database::metadataBytes()`, а
> разбивку по статьям даёт `hierarchy().memoryUse()`. Замер — ниже, в разделе
> «Производительность».
...
сбрасываются досрочно. Поэтому пиковая память не зависит от размера дампа, и
ограничения на его размер нет. От числа **сигналов** она зависит: иерархия
строится в ОЗУ и там же остаётся (см. оговорку выше).
...
Она растёт с числом сигналов: у этих 8 193 метаданные занимают единицы мегабайт
и в цифрах не видны, но на дизайне с миллионом узлов именно они станут главной
статьёй расхода — см. «Потолок метаданных» ниже.
...
Отдельный случай стенда (`--only=meta`) гоняет не поток изменений, а **широкую
развёртку массивов**: `logic [7:0] mem [0:249999]` четырьмя банками, миллион
элементов. Каждый элемент unpacked-массива — это отдельный узел иерархии со
своим потоком: виртуализации нет, `Builder` разворачивает массив поэлементно.

| величина | значение |
| --- | --- |
| метаданные на 10⁶ узлов | 220 МиБ |
| **на один узел иерархии** | **231 байт** |
| из них на карты имён (`StringMap`) | 29 % |
| на переключающийся поток (геометрия блоков) | ещё 107 байт |
| та же иерархия секцией на диске | 25,2 МиБ |
| `openStore` на таких метаданных | ~245 мс |
| RSS процесса против аналитического счёта | 266 МиБ против 220 |

Читать это так. **Материализация дороже диска в 8,7 раза**: секция не хранит ни
карт имён, ни списков детей — их заново отстраивает `decodeHierarchy` теми же
`addSignal`/`addMember`, что и парсер. **Открытие перестаёт быть бесплатным**:
0,24 мс на 1024 сигналах и 245 мс на миллионе узлов. **Дублирование имён при
этом ничего не стоит**: ключ карты — независимая копия имени узла, но `"[123]"`
укладывается в SSO и кучи не трогает; заметным дублирование становится на именах
длиннее 15 символов.

Числа аналитические (`memoryUse()` видит `capacity` контейнеров) и потому
**нижняя граница**: округления аллокатора и фрагментацию они не видят, что и
показывает колонка `rss_MiB`.
...
- **метаданные грузятся целиком**: у ленивого режима ограничена память значений,
  а иерархия — нет, и стоит она ~231 байт на узел (замер выше). Дешёвое лечение —
  заменить `StringMap` в каждом узле на отсортированный по имени массив детей с
  бинарным поиском: поиск по имени не горячий, а карты съедают 29 % счёта.
  Дорогое, зато снимающее потолок, — виртуализировать элементы массива, вычисляя
  ребёнка из `ArrayType` по требованию; сейчас это замуровано тем, что
  `Signal` — это пара (`Database*`, `NodeId`), то есть хэндлу нужен физически
  существующий узел. Браться стоит под реальный дизайн, который не открывается.
