...
значений → метаданные (иерархия, граф типов, геометрия блоков) → футер `'WSFT'`
со смещением метаданных. Спутников — сайдкаров, отдельных индексов — у него нет,
поэтому готовый store открывается заново без парсера и без исходного дампа:
