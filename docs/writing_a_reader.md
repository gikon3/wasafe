...
| метаданные: секция `'WSFH'` (иерархия и типы), секция `'WSFI'` (геометрия) | в `finish()` |
| футер (смещение и размер метаданных, их CRC32) | в `finish()`, в самом конце файла |
...
об этом разными сообщениями. Третий случай — **тихо испорченный файл**: длины
сходятся, футер на месте, но метаданные не сходятся с записанной CRC32, и об
этом тоже говорится отдельно.

Версия у файла и у каждой секции — пара major.minor. Читатель отказывается
работать при чужом major и спокойно читает больший minor: секция несёт свою
длину, поэтому дописанный будущей версией хвост пропускается. Правило для
собственных правок формата — расширять секцию **только в конец** и только с
приращением minor.

**Значения контрольной суммой по умолчанию не покрыты.** У каждого блока она
записана в индексе (`BlockRef::crc32`, рядом — `count`, число изменений), но
сверять её на каждом чтении дорого: замер на стенде показал удвоение времени
ленивого чтения. Поэтому проверка блоков — отдельный режим:

```cpp
auto db = WaSafe::openStore("dump.wsfstore", {}, /*verifyBlocks*/ true);
```

Источнику своего формата эти поля заполнять не нужно: ноль в них означает «не
задано», и проверка такие блоки пропускает.
