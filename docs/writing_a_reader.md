...
записана в индексе (`BlockRef::crc32`), но сверять её на каждом чтении дорого:
замер на стенде показал удвоение времени ленивого чтения. Поэтому проверка
блоков — отдельный режим:

```cpp
auto db = WaSafe::openStore("dump.wsfstore", {}, /*verifyBlocks*/ true);
```

Источнику своего формата это поле заполнять не нужно: ноль в нём означает «не
