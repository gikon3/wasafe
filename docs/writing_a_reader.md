...
- **до 64 бит — в `cookie`**. Он едет внутри самого индекса и переживает
  сериализацию сайдкара, так что побочная таблица не нужна. Типичное
  применение — упаковать пару `uint32` (позиция и длина цепочки изменений
  внутри чанка) или индекс в собственном массиве;
- **больше — в своих структурах**, ключуя тем же, чем ядро ключует кэш:
  `(SignalId, offset, cookie)`. Потолка по размеру там нет, но и в сайдкар это
...
больше, но курсоры и кэш придётся написать самому.
