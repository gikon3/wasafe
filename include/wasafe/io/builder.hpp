...
/// Builder, пишущий нормализованное блочное хранилище *.wsfstore. Позволяет
/// затем читать диаграмму ленивым storage, без полной загрузки в память и
/// независимо от исходного формата.
///
/// Файл САМОДОСТАТОЧЕН: вместе с блоками значений в него уезжают иерархия,
/// типы и геометрия блоков, поэтому позже он открывается через openStore()
/// без парсера и без исходного дампа (см. io/store.hpp).
///
...
/// в ОЗУ не собирается. Следствие: блоки разных потоков чередуются в файле, а
/// метаданные дописываются в finish() — прерванная ingestion оставляет файл
/// без них, и открыть его нельзя.
