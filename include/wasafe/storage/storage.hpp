...
///
/// ПОТОКИ. Storage (и Database поверх него) после сборки только читает, но
/// потокобезопасным НЕ является: LazyStorage мутирует LRU-кэш блоков прямо в
/// const-методах valueAt/nextChange/prevChange, а FileBlockSource двигает
/// позицию своего ifstream. Один экземпляр — один поток; второму потоку нужен
/// свой экземпляр поверх тех же данных.
...
public:
    ...
    ///
    /// Возвращаемый ValueView НЕ владеет данными — он указывает внутрь storage,
    /// и срок его жизни задаёт реализация:
    ///   * LazyStorage — до следующего valueAt/nextChange/prevChange/prefetch/
    ///     release на ЭТОМ ЖЕ storage: любое из них способно вытеснить блок из
    ///     LRU-кэша, а вид указывает в распакованный блок;
    ///   * MemoryStorage — пока живы данные потоков (вид указывает прямо в них).
    /// Между потоками ValueView передавать нельзя — копируйте в Value.
    ...
    /// Греет кэш ЭТОГО экземпляра: у другого экземпляра кэш свой, и прогрев в
    /// фоновом потоке ему не поможет — фоновой работе следует отдавать наверх
    /// готовые значения, а не рассчитывать на общий LRU.
    ...
    /// Объём МЕТАДАННЫХ хранилища в байтах (0 — если их нет).
    ///
    /// Дубликаты индекс РАЗДЕЛЯЮТ, поэтому сумма по экземплярам посчитала бы эту
    /// память по разу на каждый поток исполнения (см. duplicate()).
    [[nodiscard]] virtual std::size_t metadataBytes() const { return 0; }

    // --- размножение по потокам ---------------------------------------------
    /// Независимый равноценный экземпляр поверх ТЕХ ЖЕ данных: неизменяемое
    /// (индекс, значения, блоки) разделяется, изменяемое (кэш, дескриптор
    /// файла) у дубликата своё. Это не копия состояния — кэш начинается пустым.
    ///
    /// nullptr по умолчанию — реализация размножения не поддерживает.
    [[nodiscard]] virtual std::unique_ptr<Storage> duplicate() const { return nullptr; }
