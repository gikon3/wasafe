...
/// Резидентная память процесса в байтах; 0 — платформа не поддерживается.
///
/// Реализовано только для Linux (/proc/self/statm). На прочих платформах колонка
/// печатается прочерком.
[[nodiscard]] std::size_t residentBytes();

/// Вернуть аллокатору-и-ОС всё, что уже освобождено. Без этого RSS монотонно
/// растёт в пределах процесса, и дельта второго случая подряд ничего не значит:
/// glibc держит освобождённые чанки у себя. Где такого рычага нет — no-op, и
/// тогда RSS честнее читать как абсолютную величину, а не как дельту.
